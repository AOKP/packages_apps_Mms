...
public class MessagingNotification {
    ...
    private static void updateNotification(
        ...
        if (mostRecentNotification.mIsSms) {
            quickReply.putExtra("markAndClose", false);
            quickReply.putExtra("needsDeleted", false);
            // intents
            Intent textReply = new Intent(quickReply);
            Intent deleteSms = getReplyIntent(context, mostRecentNotification,
                uniqueThreadCount, notificationSet, messageCount);
            Intent markSmsRead = getReplyIntent(context, mostRecentNotification,
                uniqueThreadCount, notificationSet, messageCount);
            ...
            if (deleteSms != null && selections[2]) {
            ...
            if (markSmsRead != null && selections[3]) {
                CharSequence readText = context.getText(R.string.qr_alert_read);
                if (DEBUG)
                    Log.v(TAG, dbgOut + readText);
                noti.addAction(R.drawable.ic_sms_mms_delivered,
                    readText,
                    PendingIntent.getActivity(context,
                        new Random().nextInt(), // required to prevent cache from taking over
